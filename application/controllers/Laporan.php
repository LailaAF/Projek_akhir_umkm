...
class Laporan extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $this->form_validation->set_rules('transaksi', 'Transaksi', 'required|in_list[pembelian,pemesanan]');
        ...
        if ($this->form_validation->run() == false) {
            ...
            $query = '';
            if ($table == 'pembelian') {
                $query = $this->admin->getPembelian(null, null, ['mulai' => $mulai, 'akhir' => $akhir]);
            } else {

                $query = $this->admin->getPemesanan(null, null, ['mulai' => $mulai, 'akhir' => $akhir],  $konsumen_id);
                if ($query == Null) {
    ...
    private function _cetak($data, $table_, $tanggal)
    {
        ...
        $table = $table_ == 'pembelian' ? 'Pembelian' : 'Pesanan';
        $pdf = new FPDF();
        $pdf->AddPage('P', 'Letter');
        $pdf->SetFont('Times', 'B', 16);
        $pdf->Cell(190, 7, 'Nota Transaksi ' . $table, 0, 1, 'C');
        $pdf->SetFont('Times', '', 10);
        $pdf->Cell(190, 6, 'Aplikasi Pemesanan Produk Dari Supplier Ke Konsumen Berbasis Web 2020', 0, 1, 'C');
        ...
        if ($table_ == 'pembelian') :
            ...
            foreach ($data as $d) {
                ...
                $pdf->Cell(35, 7, $d['id_pembelian'], 1, 0, 'C');
                $pdf->Cell(55, 7, $d['nama_produk'], 1, 0, 'L');
            ...
            $pdf->Cell(35, 7, 'ID Transaksi', 1, 0, 'C');
            $pdf->Cell(35, 7, 'Nama Produk', 1, 0, 'C');
            ...
            foreach ($data as $d) {
                ...
                $pdf->Cell(35, 7, $d['id_pemesanan'], 1, 0, 'C');
                $pdf->Cell(35, 7, $d['nama_produk'], 1, 0, 'L');
                $pdf->Cell(25, 7, $d['jumlah'] . ' ' . $d['nama_satuan'], 1, 0, 'C');
                $pdf->Cell(25, 7, number_format($d['harga']), 1, 0, 'C');

                $hasil = $d['jumlah'] * $d['harga'];
                $total = $hasil + $total;
